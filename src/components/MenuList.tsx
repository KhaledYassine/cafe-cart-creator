...
const MenuList = ({
  ...
  return <section className="py-8 bg-secondary/30">
      <div className="container px-4 mx-auto">
        {filteredCategories.length === 0 ? <div className="text-center py-12">
          ...
          return <div key={category.id} className="bg-card rounded-lg border shadow-sm overflow-hidden">
              ...
              if (value) handleToggle(category.id);else handleToggle(category.id);
            }}>
                    <AccordionItem value={category.id} className="border-none">
                      <AccordionTrigger className="px-6 py-4 hover:no-underline bg-secondary/20 rounded-none text-base font-normal">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-md overflow-hidden shadow-sm flex-shrink-0">
                            <img src={getCategoryImage(category.id)} alt={category.name} className="w-full h-full object-cover" />
                          </div>
                      ...
                      <AccordionContent>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                          {categoryItems.map(item => <MenuItem key={item.id} item={item} addToCart={addToCart} />)}
                        </div>
                      </AccordionContent>
                    </AccordionItem>
                  </Accordion>
